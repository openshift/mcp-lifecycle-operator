...
1. Update or create a `Snapshot` with the image digest and source revision you
   want to promote. The checked-in override snapshot intentionally uses
   `REPLACE_WITH_KONFLUX_IMAGE_DIGEST`; replace it with the digest from a
   successful `mcp-lifecycle-operator-release-010` Konflux build before
   applying it.
...
- The stage resources are for the `mcp-lifecycle-operator-01` Konflux
  application in the `mcp-lifecycle-operator-tenant` namespace.
