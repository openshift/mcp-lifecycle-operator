...
var _ = Describe("MCPServer Gateway Controller", func() {
			...
			Spec: mcpv1alpha1.MCPServerSpec{
				Source: mcpv1alpha1.Source{
					...
					ContainerImage: &mcpv1alpha1.ContainerImageSource{
						Ref: "ghcr.io/test/server:latest",
					},
				},
	...
	Context("When credential-ref annotation is set", func() {
		const name = "gw-test-cred-ref"
		...
		It("should set credentialRef with default key 'token'", func() {
			mcpServer := newMCPServer(name, map[string]string{
				AnnotationGatewayRef:           "gateway-system/mcp-gateway",
				AnnotationGatewayCredentialRef: "my-secret",
			})
			Expect(k8sClient.Create(ctx, mcpServer)).To(Succeed())
			...
			regSpec := reg.Object["spec"].(map[string]any)
			credRef := regSpec["credentialRef"].(map[string]any)
			Expect(credRef["name"]).To(Equal("my-secret"))
			Expect(credRef["key"]).To(Equal("token"))
		})

		It("should use custom key from credential-key annotation", func() {
			mcpServer := newMCPServer(name, map[string]string{
				AnnotationGatewayRef:           "gateway-system/mcp-gateway",
				AnnotationGatewayCredentialRef: "my-secret",
				AnnotationGatewayCredentialKey: "api-key",
			})
			Expect(k8sClient.Create(ctx, mcpServer)).To(Succeed())
			...
			credRef := regSpec["credentialRef"].(map[string]any)
			Expect(credRef["name"]).To(Equal("my-secret"))
			Expect(credRef["key"]).To(Equal("api-key"))
		})

		It("should not set credentialRef when annotation is absent", func() {
			mcpServer := newMCPServer(name, map[string]string{
				AnnotationGatewayRef: "gateway-system/mcp-gateway",
			})
			Expect(k8sClient.Create(ctx, mcpServer)).To(Succeed())
			...
			regSpec := reg.Object["spec"].(map[string]any)
			_, hasCredRef := regSpec["credentialRef"]
			Expect(hasCredRef).To(BeFalse())
		})
	})

	Context("When spec.config.path is set", func() {
		...
		It("should use spec.config.path in HTTPRoute path match", func() {
			mcpServer := newMCPServer(name, map[string]string{
				AnnotationGatewayRef: "gateway-system/mcp-gateway",
			})
			mcpServer.Spec.Config.Path = "/sse"
			Expect(k8sClient.Create(ctx, mcpServer)).To(Succeed())
